...
{
    [FormerlySerializedAs("CollapseRenderersIntoTopLevelGameObject")] [Tooltip("Collapse all renderers into their top level gameObject. If a gameObject hierarchy exists that has colliders/renderers/animators/etc at multiple levels, they will all be represented by a single entry in the state.  This defaults to True as it is normally desired to see each player, car, building, etc as a single entry in the state.  However, it can be useful to set this to false in cases where you want to validate individual render bounds, colliders, rigibodies, etc on individual armatures, weapons, or other components that are children in the hierarchy.")]
    public bool collapseRenderersIntoTopLevelGameObject = true;
    
    [FormerlySerializedAs("IncludeOnlyOnCameraObjects")] [Tooltip("Include only objects at are within the render bounds of the main camera view.")]
    public bool includeOnlyOnCameraObjects = true;
    
    [FormerlySerializedAs("CollectStateFromBehaviours")] [Tooltip("(WARNING: Performance Impact) Include field/property values for behaviours.")]
   ...
   private string FastTrim(string input)
    ...
    public List<RenderableGameObjectState> GetStateForCurrentFrame()
    {
        ...
        var statefulUIObjects = FindObjectsByType<CanvasRenderer>(FindObjectsSortMode.None).Select(r => r.gameObject);
        ...
        foreach (var r in rendererObjects)
        {
            ...
            while (nextParent != null)
            {
                ...
                if (_transformsIveSeen.TryGetValue(nextParent, out var nextParentStatus) && nextParentStatus.HasKeyTypes != null)
                {
